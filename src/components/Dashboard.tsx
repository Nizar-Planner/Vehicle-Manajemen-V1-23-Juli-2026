...
import { UioUnit, SparePart, WorkshopBooking, DashboardStats, FleetKPIs } from '../types';
...
export default function Dashboard({ 
  ...
  return (
    <div className="space-y-6" id="dashboard-container">
      ...
      {/* Metrik Indikator Kinerja Armada Pertambangan (MTTR, MTBF, MA, PA) */}
      <div className="bg-slate-900 text-white p-5 rounded-3xl border border-slate-800 shadow-xl space-y-4" id="fleet-kpi-metrics-section">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-sm font-extrabold uppercase font-mono tracking-wider text-slate-100 flex items-center gap-2">
                Key Performance Indicators (KPI) Armada Pertambangan
              </h2>
              <p className="text-xs text-slate-400 font-sans">
                Pengukuran real-time jam keandalan perbaikan (MTTR), durasi operasi antarkerusakan (MTBF), serta ketersediaan fisik (PA) dan mekanis (MA).
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="px-3 py-1.5 bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-black text-xs uppercase font-mono rounded-xl transition-all shadow-xs flex items-center gap-1.5 self-start sm:self-auto cursor-pointer"
          >
            <Plus size={14} />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-slate-800/80 p-4 rounded-2xl border border-slate-700/80 space-y-2">
            <div className="flex items-center justify-between text-xs text-slate-400 font-medium">
              <span className="font-mono uppercase text-[11px] font-bold text-slate-300">MTTR (Mean Time To Repair)</span>
              <span className="text-[10px] bg-slate-700 text-slate-300 px-1.5 py-0.5 rounded-md font-mono">Std: &lt; 4.0 Jam</span>
            </div>
            <div className="flex items-baseline justify-between">
              <span className="text-3xl font-mono font-extrabold text-amber-400">{fleetKpis.mttrHours}</span>
              <span className="text-xs font-mono text-slate-400">Jam / Perbaikan</span>
            </div>
            <div className="w-full bg-slate-700/60 h-2 rounded-full overflow-hidden">
              <div 
                className={`h-full ${fleetKpis.mttrHours <= 4 ? 'bg-emerald-400' : 'bg-amber-400'}`}
                style={{ width: `${Math.min(100, (fleetKpis.mttrHours / 8) * 100)}%` }}
              ></div>
            </div>
            <p className="text-[11px] text-slate-400 leading-tight">
              Rata-rata durasi pengerjaan servis per kerusakan.
            </p>
          </div>
          ...
          <div className="bg-slate-800/80 p-4 rounded-2xl border border-slate-700/80 space-y-2">
            <div className="flex items-center justify-between text-xs text-slate-400 font-medium">
              <span className="font-mono uppercase text-[11px] font-bold text-slate-300">MTBF (Mean Time Between Failures)</span>
              <span className="text-[10px] bg-slate-700 text-slate-300 px-1.5 py-0.5 rounded-md font-mono">Std: &gt; 100 Jam</span>
            </div>
            <div className="flex items-baseline justify-between">
              <span className="text-3xl font-mono font-extrabold text-emerald-400">{fleetKpis.mtbfHours}</span>
              <span className="text-xs font-mono text-slate-400">Jam Operasi / Breakdown</span>
            </div>
            <div className="w-full bg-slate-700/60 h-2 rounded-full overflow-hidden">
              <div 
                className="h-full bg-emerald-400"
                style={{ width: `${Math.min(100, (fleetKpis.mtbfHours / 200) * 100)}%` }}
              ></div>
            </div>
            <p className="text-[11px] text-slate-400 leading-tight">
              Rata-rata jam operasi bebas gangguan antarkerusakan.
            </p>
          </div>
          ...
          <div className="bg-slate-800/80 p-4 rounded-2xl border border-slate-700/80 space-y-2">
            <div className="flex items-center justify-between text-xs text-slate-400 font-medium">
              <span className="font-mono uppercase text-[11px] font-bold text-slate-300">MA (Mechanical Availability)</span>
              <span className="text-[10px] bg-slate-700 text-slate-300 px-1.5 py-0.5 rounded-md font-mono">Target: &gt; 90%</span>
            </div>
            <div className="flex items-baseline justify-between">
              <span className="text-3xl font-mono font-extrabold text-blue-400">{fleetKpis.mechanicalAvailability}%</span>
              <span className="text-xs font-mono text-slate-400">Jam Op / (Op + Repair)</span>
            </div>
            <div className="w-full bg-slate-700/60 h-2 rounded-full overflow-hidden">
              <div 
                className={`h-full ${fleetKpis.mechanicalAvailability >= 90 ? 'bg-blue-400' : 'bg-rose-400'}`}
                style={{ width: `${Math.min(100, fleetKpis.mechanicalAvailability)}%` }}
              ></div>
            </div>
            <p className="text-[11px] text-slate-400 leading-tight">
              Kesiapan mekanis armada saat dibutuhkan beroperasi.
            </p>
          </div>
          ...
          <div className="bg-slate-800/80 p-4 rounded-2xl border border-slate-700/80 space-y-2">
            <div className="flex items-center justify-between text-xs text-slate-400 font-medium">
              <span className="font-mono uppercase text-[11px] font-bold text-slate-300">PA (Physical Availability)</span>
              <span className="text-[10px] bg-slate-700 text-slate-300 px-1.5 py-0.5 rounded-md font-mono">Target: &gt; 92%</span>
            </div>
            <div className="flex items-baseline justify-between">
              <span className="text-3xl font-mono font-extrabold text-teal-400">{fleetKpis.physicalAvailability}%</span>
              <span className="text-xs font-mono text-slate-400">Kesiapan Fisik Total</span>
            </div>
            <div className="w-full bg-slate-700/60 h-2 rounded-full overflow-hidden">
              <div 
                className={`h-full ${fleetKpis.physicalAvailability >= 92 ? 'bg-teal-400' : 'bg-amber-400'}`}
                style={{ width: `${Math.min(100, fleetKpis.physicalAvailability)}%` }}
              ></div>
            </div>
            <p className="text-[11px] text-slate-400 leading-tight">
              Kesiapan fisik total armada terhadap 24 jam kalender site.
            </p>
          </div>
        </div>
      </div>
